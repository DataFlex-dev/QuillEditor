...
class TableWidget {
	...
	addWidget() {
		...
		const tableModule = quill.getModule("table");
		let button = container.querySelector(".wi-menu-button");
		let menu = container.querySelector(".wi-menu");
		let addRow = container.querySelector(".wi-add-row");
		let addColumn = container.querySelector(".wi-add-column");
		let deleteRow = container.querySelector(".wi-delete-row");
		let deleteColumn = container.querySelector(".wi-delete-column");

		button.addEventListener("pointerenter", () => {
			container.querySelector(".wi-menu").style.visibility = "visible";
		});
		menu.addEventListener("pointerleave", () => {
			container.querySelector(".wi-menu").style.visibility = "hidden";
		});
	...
	getToolbarButton(options) {
		...
		for (let i = d[1]; i > 0; i--) {
			for (let j = d[0]; j > 0; j--) {
				...
				createButton.addEventListener("pointerenter", () => {
					tooltip.querySelector(".wi-tooltip-header").innerText = `New Table: ${j}x${i}`;
				});
				createButton.addEventListener("pointerleave", () => {
					tooltip.querySelector(".wi-tooltip-header").innerText = `New Table`;
				});
		...
		button.addEventListener("click", () => {
			span.querySelector(".wi-tooltip").style.visibility = "visible";
		});
		tooltip.addEventListener("pointerleave", (e) => {
			span.querySelector(".wi-tooltip").style.visibility = "hidden";
		});
		return span;
	}
}